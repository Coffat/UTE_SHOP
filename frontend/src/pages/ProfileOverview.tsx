          ...
          <div className="rounded-2xl border border-white/60 bg-pure-ivory/55 p-2">
            {quickActions.map((action, idx) => (
              <button
                ...
                type="button"
                onClick={() => {
                  if (idx === 0) setEditingProfile((v) => !v);
                }}
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {dynamicStatCards.map((card) => (
          ...
          >
            ...
            <button
              type="button"
              className="mt-2 inline-flex items-center gap-1 text-xs font-semibold text-primary hover:text-deep-plum transition"
            >
              {card.link}
      ...
      <div className="grid gap-4 xl:grid-cols-[1fr_360px]">
        <article className="glass-panel rounded-[22px] p-5 shadow-[0_10px_40px_rgba(168,85,247,0.04)]">
          <div className="mb-4 flex items-center justify-between">
            ...
            <button type="button" className="text-sm font-medium text-primary hover:text-deep-plum transition">
              Xem tất cả
            </button>
          </div>
          ...
          {ordersStatus === "loading" ? (
            ...
            <ul className="space-y-3">
              {recentOrdersList.map((order) => (
                <li
                  key={order.id}
                  className="flex items-center gap-3 rounded-2xl border border-white/60 bg-pure-ivory/65 px-4 py-3 hover:bg-white transition-colors"
                >
          ...
          <button type="button" className="mt-4 text-sm font-semibold text-primary hover:text-deep-plum transition">
            Xem tất cả đơn hàng
          </button>
        </article>
        ...
        <div className="space-y-4">
          ...
          >
            ...
            <button
              type="button"
              className="login-gradient-bg relative mt-5 inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold text-pure-ivory shadow-md transition-transform group-hover:-translate-y-0.5"
            >
              Khám phá ngay
