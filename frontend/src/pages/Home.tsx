...
import { fetchHomeProducts, BackendProduct } from "@/features/catalog/catalogSlice";
...
const mapToUI = (p: BackendProduct, fallbackImage: string): Product => {
  ...
  if (tags?.some((t) => t.slug === 'ban-chay')) {
    badge = { label: "Bán chạy", tone: "default" };
  } else if (tags?.some((t) => t.slug === 'moi-ve')) {
    badge = { label: "Mới", tone: "pink" };
  } else if (tags?.some((t) => t.slug === 'khuyen-mai')) {
...
export function Home() {
  ...
  const { homeProducts } = useAppSelector((state) => state.catalog);
