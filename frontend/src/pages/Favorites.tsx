...
export function Favorites() {
  ...
  // Giả lập danh sách yêu thích bằng vài sản phẩm từ catalog (trong thực tế sẽ lấy từ wishlistSlice)
  const { products } = useSelector((state: RootState) => state.catalog);
  
  // Tạm thời để mảng rỗng để hiển thị Empty State theo yêu cầu của Neo-Glassmorphism
  // Nếu muốn xem danh sách thì đổi thành: const favoriteItems = products.slice(0, 4);
          ...
          {favoriteItems.map((p) => {
            ...
            return (
              ...
              >
                ...
                <button className="absolute right-6 top-6 z-10 w-9 h-9 rounded-full bg-white/80 backdrop-blur-md shadow-sm border border-white flex items-center justify-center text-primary hover:bg-rose-50 transition-colors">
